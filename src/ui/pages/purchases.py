...
class AddPurchaseDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.rows.setHorizontalHeaderLabels(["Item", "Category", "Qty Secondary (slab/box)", "Qty Primary (sqft/piece)", ""])
    ...
    def add_line(self):
        ...
        def on_item_change():
            ...
            if cat in ("SLAB", "TILE"):
                ...
                # auto-calc sqft if sqft_per_unit available
                def recalc():
                    if it.sqft_per_unit:
                        try:
                ...
                # BLOCK/TABLE: only primary (piece) matters
                qty_sec.setValue(0)
                qty_sec.setEnabled(False)
                qty_pri.setEnabled(True)

        def remove_row():
            self.rows.removeRow(r)
