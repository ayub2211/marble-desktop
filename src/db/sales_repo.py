...
def create_sale(db, payload: dict) -> Sale:
    ...
    for r in rows:
        ...
        prepared.append((item, _to_float(qty_primary), (None if qty_secondary is None else _to_int(qty_secondary))))
    ...
    for (item, qty_primary, qty_secondary) in prepared:
        ...
        db.add(SaleItem(
            sale_id=s.id,
            item_id=item.id,
            qty_primary=float(qty_primary or 0),
            qty_secondary=(None if cat in ("BLOCK", "TABLE") else (None if qty_secondary is None else int(qty_secondary))),
            unit_primary=unit_primary,
            unit_secondary=unit_secondary
        ))

        # ledger (SALE negative)
        add_ledger_entry(
            db=db,
            item_id=item.id,
            ...
            qty_secondary=(-int(qty_secondary) if qty_secondary is not None and cat in ("SLAB", "TILE") else None),
            unit_primary=unit_primary,
            unit_secondary=unit_secondary,
            ref_type="sale",
            ref_id=s.id
...
def cancel_sale(db, sale_id: int):
    """
    Reverses a sale by ADDING opposite ledger + inventory entries.
    (We do NOT delete history; we restore stock.)
    """
    sale = get_sale_details(db, int(sale_id))
    if not sale:
        raise ValueError("Sale not found.")

    location_id = sale.location_id
    if not location_id:
        raise ValueError("Sale location missing.")
    ...
    for line in items:
        ...
        cat = (item.category or "").upper()
        unit_primary = line.unit_primary or item.unit_primary or ("piece" if cat in ("BLOCK", "TABLE") else "sqft")
        unit_secondary = line.unit_secondary or item.unit_secondary

        qty_primary = float(line.qty_primary or 0)
        qty_secondary = None if line.qty_secondary is None else int(line.qty_secondary or 0)
        ...
        add_ledger_entry(
            ...
            qty_secondary=(qty_secondary if cat in ("SLAB", "TILE") else None),
            unit_primary=unit_primary,
            unit_secondary=unit_secondary,
        ...
        note_text = f"SaleCancel#{sale.id}" + (f" — {customer}" if customer else "")
    ...
    # optional flag if your model has it
    try:
        if hasattr(sale, "is_cancelled"):
            sale.is_cancelled = True
        if hasattr(sale, "cancelled_at"):
            sale.cancelled_at = None  # set if you have datetime fields
    except Exception:
        pass
